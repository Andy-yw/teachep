                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <div class="form-group">
                            ...
                            <input type="type" class="form-control" name="article_name" id="article_name" value="{{$data['article_name'] }}" placeholder="请输入文章标题">
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="article_type_id" id="article_type_id" style="width: 100%;">
                                @foreach($articletype as $k => $v)
                                    <option value="{{ $v->id }}"  @if($v->id==$data->article_type_id) selected="selected" @endif>
                                        {{ $v->article_type_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <img width="220"  height="220" id="upimg" src="{{ asset($data['article_img']) }}" alt="Avatar">
                            <a onclick="addimg()"class="btn btn-primary">添加图片</a>
                            <input class="form-control" type="text" id="article_img" name="article_img" value="{{$data['article_img'] }}" style="display: none;">
                        </div>
                        <div class="form-group">
                            <label for="exampleInput">文章介绍</label>

                            <textarea class="form-control"  name="article_introduction" id="article_introduction">{{$data['article_introduction'] }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="exampleInput">文章内容</label>
                            <div id="editor">{{$data['article_text'] }}</div>
                        </div>
                    </div>
...
@section('js')
    ...
    <script>
        ...
        $("#addit").click(function() {
            ...
            var id=$.trim($("#id").val());
            $.post("{{ url('admin/article/update', [$data->id]) }}",{
